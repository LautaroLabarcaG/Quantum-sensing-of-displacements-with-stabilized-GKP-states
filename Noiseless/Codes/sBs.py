...
def sBs_allbits(Delta,q0,p0,sensor,T,gauges):
    ...
    def prob_from_b(b,rho):
        b = int_to_r_bits(b,2*T)
        Krauss = Krauss_from_b(b)
        return np.real((Krauss.getH()@Krauss@rho).diagonal().sum())

    probs = Parallel(n_jobs=16)(delayed(prob_from_b)(b,rho) for b in range(2**(2*T)))
...
def sBs_allqbits(Delta,q0,p0,sensor,T,gauges):
    ...
    probs = Parallel(n_jobs=-1)(delayed(prob_from_b)(bq,rho) for bq in range(2**(T)))
    # probs = []
    # for bq in range(2**T):
...
def sBs_allpbits(Delta,q0,p0,sensor,T,gauges):
    ...
    def prob_from_b(bq,rho):
        ...
        for j in range(len(bq)):
            ...
            KgqU, KeqU = Krauss_dictionary_U[f'Kgq{mu_q}'], Krauss_dictionary_U[f'Keq{mu_q}']
            rho = KgqU @ rho @ KgqU.getH() + KeqU @ rho @ KeqU.getH()
            mu_p = (mu_p +1)% 2#gauge update

            KgpU, KepU = Krauss_dictionary_U[f'Kgp{mu_p}'], Krauss_dictionary_U[f'Kep{mu_p}']
            if bq[j] == '0':
                rho = KgpU @ rho @ KgpU.getH()
            else:
                rho = KepU @ rho @ KepU.getH()
            mu_q = (mu_q +1)% 2#gauge update
        return (rho).diagonal().sum()
...
def sBs_allqbits_center(Delta,q0,p0,sensor,T,gauges):
    """
    R is the number of q-p cycles
    rho0 the initial state of the oscillator 
    gauges: starting gauge
    returns the expectation value of q for all q bitstrings.
    """
    beta = (q0 + 1j*p0)/np.sqrt(2)
    U_q0p0 = Displacement(beta)
    rho0 = U_q0p0@sensor@U_q0p0.getH()
    ...
    def center_from_b(bq,rho):
        ...
        rho = rho/np.real((rho).diagonal().sum())
        return (q_op @ rho).diagonal().sum()

    exp_q = Parallel(n_jobs=16)(delayed(center_from_b)(bq,rho) for bq in range(2**(T)))
    # probs = []
    # for bq in range(2**T):
    #     probs.append(prob_from_b(bq,rho))
    return np.real(np.array(exp_q))
